...
//     let PersonArr = [];
//     for(key of people){
//         if(key.isSocialDistancing == false) {
//             for(friends of key.friends) {
//                 if(friends.isSocialDistancing == false && friends.hasCovid) {
//                     PersonArr.push(key.firstName + " " + key.lastName);
//                     break;
//                 }
//             }
//         }
...
function coronaVirusAtRisk(persons) {
    ...
    for (let i = 0; i < persons.length; i++) {
        ...
        for (let j = 0; j < persons[i].friends.length; j++) {
            if (!persons[i].friends[j].isSocialDistancing && persons[i].friends[j].hasCovid) {
                atRisk.push(persons[i].firstName + " " + persons[i].lastName);
                break;
            }
        }
    }
